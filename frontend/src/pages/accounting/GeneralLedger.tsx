...
export default function GeneralLedger() {
  ...
  const printReport = () => {
    ...
    const toLabel = to || "--";
    const accLabel = account ? `${account.code} · ${account.name}` : (accountCode === "all" ? "All Accounts Ledger" : accountCode);

    const rowsHtml = rows.map(r => `
      <tr>
        <td>${new Date(r.date).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: '2-digit' })}</td>
        <td style="font-family: monospace; font-size: 10px;">${r.refNo || "SYS-GEN"}</td>
        <td>${r.memo || "NO_NARRATIVE"}</td>
        <td class="num">${r.debit > 0 ? Number(r.debit).toLocaleString(undefined, { minimumFractionDigits: 2 }) : "-"}</td>
        <td class="num">${r.credit > 0 ? Number(r.credit).toLocaleString(undefined, { minimumFractionDigits: 2 }) : "-"}</td>
    ...
    const html = `
      <html>
        <head>
          <title>General Ledger - ${companyName}</title>
          <style>
        ...
        <body>
          ...
          <div class="info-box">
            <div class="info-label">Account Registry</div>
            <div class="info-value">${accLabel}</div>
          </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style="width: 33%">Operational Narrative</th>
          ...
          <div class="footer">
            Corporate Registry Audit • Mind Spire Financial Core 2.0 • Generated on ${new Date().toLocaleString()}
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <Badge className="bg-indigo-100 text-indigo-700 border-indigo-200 text-[10px] font-bold uppercase tracking-widest px-3 py-1">Transactional Registry</Badge>
          ...
          <div className="bg-slate-900 p-8 flex items-center justify-between border-b border-white/5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-indigo-400 text-[9px] font-bold uppercase tracking-[0.3em] mt-1">Registry Selection Protocol</p>
              </div>
            </div>
            <div className="flex gap-4">
              {account && (
                <div className="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-xl border border-white/10">
                  ...
                  <span className="text-[10px] font-bold text-white uppercase tracking-widest">{account.code} · {account.name}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <CardContent className="p-8 sm:p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 items-end">
              <div className="lg:col-span-4 space-y-3">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 ml-1">Ledger Registry</Label>
                <Popover open={accountOpen} onOpenChange={setAccountOpen}>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="uppercase tracking-tight truncate">
                          {accountCode === "all" ? "All Accounts Ledger" : (accountCode ? accounts.find(a => a.code === accountCode)?.name || accountCode : "Select Account")}
                        </span>
                      </div>
                  ...
                  <PopoverContent className="w-[400px] p-0 shadow-2xl border-slate-200 rounded-2xl overflow-hidden" align="start">
                    <Command>
                      <CommandInput placeholder="Search registry..." className="h-12 border-none focus:ring-0" />
                      <CommandList className="max-h-[300px]">
                        <CommandEmpty>No account found.</CommandEmpty>
                        <CommandGroup>
                          <CommandItem
                            value="all All Accounts Ledger"
                            onSelect={() => {
                              setAccountCode("all");
                              setAccountOpen(false);
                            }}
                            className="py-3 px-4 cursor-pointer hover:bg-indigo-50 border-b border-slate-100"
                          >
                            <div className="flex items-center w-full gap-3">
                              <Check className={cn("h-4 w-4 text-indigo-600", accountCode === "all" ? "opacity-100" : "opacity-0")} />
                              <span className="font-mono text-xs text-indigo-400 min-w-[50px]">ALL</span>
                              <span className="font-black text-indigo-600 uppercase tracking-tight truncate">All Accounts Ledger</span>
                            </div>
                          </CommandItem>
                          {accounts.map(acc => (
                            <CommandItem
                            ...
                            >
                              <div className="flex items-center w-full gap-3">
                                <Check className={cn("h-4 w-4 text-indigo-600", accountCode === acc.code ? "opacity-100" : "opacity-0")} />
                                <span className="font-mono text-xs text-slate-400 min-w-[50px]">{acc.code}</span>
                                <span className="font-bold text-slate-700 uppercase tracking-tight truncate">{acc.name}</span>
                              </div>
                            </CommandItem>
                          ))}
                        </CommandGroup>
              ...
              <div className="lg:col-span-2">
                <Button 
                  variant="outline" 
                  onClick={() => { setAccountCode(""); setFrom(""); setTo(""); setRows([]); setAccount(null); }}
                ...
                <table className="w-full border-separate border-spacing-y-2">
                  <thead>
                    <tr>
                      ...
                      <th className="py-6 px-6 text-left text-[10px] font-bold uppercase tracking-[0.3em] text-slate-400">Voucher</th>
