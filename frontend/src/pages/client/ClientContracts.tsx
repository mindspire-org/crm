...
export default function ClientContracts() {
  ...
  const load = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/client/contracts`, { headers });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">View contracts and their status.</p>
        </div>
        <div className="flex items-center gap-2">
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search contracts..." className="w-[220px]" />
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Contract Date</TableHead>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((r) => (
                  <TableRow key={r._id}>
                    ...
                    <TableCell>
                      <Badge variant={String(r?.status || "").toLowerCase() === "closed" ? "default" : "secondary"}>
                        {r.status || "-"}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">${Number(r.amount || 0).toLocaleString()}</TableCell>
                    <TableCell className="text-right text-muted-foreground">{toIsoDate(r.contractDate || r.createdAt)}</TableCell>
