...
router.get("/", authenticate, async (req, res) => {
  try {
    ...
    const items = await Comment.find({ projectId }).sort({ createdAt: -1 }).lean();
    res.json(items);
  } catch (e) {
...
router.post("/", authenticate, async (req, res) => {
  try {
    ...
    const doc = await Comment.create(payload);
...
router.put("/:id", authenticate, async (req, res) => {
  try {
    const existing = await Comment.findById(req.params.id).lean();
    ...
    const updates = {
      ...
      kind: req.body?.kind != null ? String(req.body.kind) : existing.kind,
    };

    const doc = await Comment.findByIdAndUpdate(req.params.id, updates, { new: true });
